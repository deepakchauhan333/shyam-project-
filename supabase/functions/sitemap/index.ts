...
Deno.serve(async (req) => {
  ...
  const path = url.pathname === '/sitemap.xml' ? 'sitemap' : url.pathname.split('/').pop()?.replace('.xml', '');

  try {
    switch (path) {
      case 'sitemap': {
        // Main sitemap index
        const sitemaps = ['main', 'categories', 'tools', 'agents', 'search'];
